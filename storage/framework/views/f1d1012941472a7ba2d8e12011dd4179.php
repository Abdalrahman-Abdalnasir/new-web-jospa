...
<body class="bg-white">
    ...
    <div class="position-relative" style="height: 290.79px;">
        <img src="<?php echo e(asset('images/spa/Hero_2.png')); ?>" alt="Contact Hero" class="w-100 h-100" style="object-fit: cover;">
    ...
    <!-- Page Content -->
        <div class="container py-5" style="min-height: 60vh;">
            <?php if($cartItems->count()): ?>
                ...
                <div class="cart-title"><?php echo e(__('messagess.cart')); ?></div>
                        ...
                        <?php $__currentLoopData = $item->services; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="service-card">
                                <div class="service-details">
                                    <div class="service-name"><?php echo e($service->service_name ?? '-'); ?></div>
                                    <div class="service-info"><?php echo e(__('messagess.employee')); ?>: <?php echo e($service->employee->full_name ?? '-'); ?></div>
                                    <div class="service-info"><?php echo e(__('messagess.branch')); ?>: <?php echo e($item->branch->name ?? '-'); ?></div>
                                    <div class="service-date"><?php echo e(__('messagess.date')); ?>:<?php echo e(\Carbon\Carbon::parse($item->start_date_time)->format('Y-m-d')); ?> - <?php echo e(\Carbon\Carbon::parse($item->start_date_time)->format('H:i')); ?></div>
                                        ...
                                        <?php if($service->discount_amount && $service->discount_amount > 0): ?>
                                            <div>
                                                <span class="original-price text-muted" style="text-decoration: line-through;">
                                                    <?php echo e($service->service_price); ?> SR
                                                </span>
                                                <span class="discounted-price text-success fw-bold">
                                                    <?php echo e($service->service_price - $service->discount_amount); ?> SR
                                                </span>
                                            </div>
                                            <div class="discount-info text-primary small">
                                                <?php echo e(__('messagess.discount')); ?>: - <?php echo e($service->discount_amount); ?> SR
                                            </div>
                                            <?php if($service->coupon_code): ?>
                                                <div class="coupon-code small text-secondary">
                                                    <?php echo e(__('messagess.coupon')); ?>: <?php echo e($service->coupon_code); ?>

                                                </div>
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <span class="original-price fw-bold"><?php echo e($service->service_price); ?> SR</span>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <?php if($service->coupon_code): ?>
                                    <div class="service-coupon">
                                        <button class="disabled" disabled><?php echo e(__('messagess.apply_coupon')); ?></button>
                                        <input type="text" value="<?php echo e($service->coupon_code); ?>" placeholder="<?php echo e(__('messagess.enter_coupon')); ?>"data-service-id="<?php echo e($service->service_id); ?>"data-booking-id="<?php echo e($item->id); ?>"disabled>
                                    </div>
                                <?php else: ?>
                                    <div class="service-coupon">
                                        <button onclick="checkCoupon(this)"><?php echo e(__('messagess.apply_coupon')); ?></button>
                                        <input type="text" placeholder="<?php echo e(__('messagess.enter_coupon')); ?>"data-service-id="<?php echo e($service->service_id); ?>"data-booking-id="<?php echo e($item->id); ?>">
                                    </div>
                                <?php endif; ?>
                ...
                <div class="summary-section">
                    <div class="summary-coupon">
                        <button onclick="checkInvoiceCoupon(this)"><?php echo e(__('messagess.add_invoice_coupon')); ?></button>
                        <input type="text" id="invoiceCouponInput" placeholder="<?php echo e(__('messagess.enter_invoice_coupon')); ?>">
                    </div>
                </div>
                    <div style="display: flex;gap: 14px;align-items: center;">
                        <?php echo e(__('messages.apply_loyalty_points')); ?>

                        <label class="switch">
                          <input type="checkbox" id="toggleBtn">
                          <span class="slider"></span>
                          <span class="label-yes"><?php echo e(__('messages.yes')); ?>   </span>
                          <span class="label-no"><?php echo e(__('messages.no')); ?></span>
                        </label>
                    </div>
                <form action="<?php echo e(route('cart.payment')); ?>" method="post">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="discount_amount" id="discount_amount" value="<?php echo e($discountTotal); ?>">
                <input type="hidden" name="loyalty_discount" id="loyalty_discount" value="0">
                <input type="hidden" name="final_total" id="final_total" value="<?php echo e($finalPrice); ?>"> <!-- Done -->
                
                <div class="summary-info">
                    <span class="summary-label"><?php echo e(__('messagess.total')); ?> :</span>
                    <span class="summary-value">SR <span id="tl"><?php echo e($finalPrice); ?></span></span>
                </div>
                <div class="wallet-label">
                    <?php echo e(__('messagess.wallet_balance')); ?> : <span class="wallet-value">SR <?php echo e($balance); ?></span>
                </div>
                <div class="pay-options">
                    <label class="pay-label">
                        <input type="radio" class="pay-radio" name="pay" value="card" checked><?php echo e(__('messagess.pay_by_card')); ?>

                    </label>
                
                    <label class="pay-label">
                        <input type="radio" class="pay-radio" name="pay" value="wallet"><?php echo e(__('messagess.pay_by_wallet')); ?>

                    </label>
                </div>
                <button class="pay-btn"><?php echo e(__('messagess.continue_to_pay')); ?> <i class="fas fa-credit-card"></i> </button>
            </div>
            <?php else: ?>
...
<script>
    function checkCoupon(button) {
        ...
        fetch(url)
            ...
            .then(data => {
                if (data.valid) {  toastr.success("<?php echo e(__('messagess.coupon_applied')); ?>: " + " " + couponCode);  } else {  toastr.error("<?php echo e(__('messagess.invalid_coupon_for_service')); ?>");  }
            })
        ...
        fetch(url)
            ...
            .then(data => {
                if (data.valid) {
                    ...
                    discountInfo.innerText = `- ${discount} SR <?php echo e(__('messagess.discount')); ?>`;
    ...
    document.getElementById('toggleBtn').addEventListener('change', function() {
        ...
        if (isChecked) {
            fetch(`<?php echo e(route('loyalty.check')); ?>`)
            .then(response => response.json())
            .then(data => {
                let loyaltyAmount = parseFloat(data.points);
                
                if (loyaltyAmount <= 0) {
                    toastr.error("<?php echo e(__('messages.insufficient_loyalty_balance')); ?>"); // الرصيد غير كافي
            ...
            toastr.success("<?php echo e(__('messages.loyalty_points_applied')); ?>: -" + appliedLoyalty + " SR");
        })
        .catch(() => {
            toastr.error("<?php echo e(__('messagess.error_occurred')); ?>");
        });
}
 else {
            ...
            toastr.info("<?php echo e(__('messages.loyalty_points_removed')); ?>");
        }
    });
</script>
...
<?php /**PATH /home/tayasmart/jospa.tayasmart.com/resources/views/components/frontend/cart.blade.php ENDPATH**/ ?>